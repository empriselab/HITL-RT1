...
class _TransformerLayer(tf.keras.layers.Layer):
  ...
  def __init__(self,
               ...
               dropout_rate: float = dropout_settings.TRANSFORMER_FF_DROPOUT,
    ...
    self.ff = tf.keras.layers.Dense(feed_forward_size, kernel_regularizer=kernel_regularizer)
...
class Transformer(tf.keras.layers.Layer):
  ...
  def __init__(self,
               ...
               return_attention_scores: bool = False,
               kernel_regularizer: Optional[tf.keras.regularizers.Regularizer] = None,
               embedding_dropout_rate: float = dropout_settings.TRANSFORMER_EMBEDDING_DROPOUT):
    ...
    Args:
      ...
      return_attention_scores: Return attention scores.
      kernel_regularizer: Regularizer for dense kernels.
      embedding_dropout_rate: Dropout rate for token and position embeddings.
    """
    ...
    self._token_emb = tf.keras.layers.Dense(feed_forward_size, kernel_regularizer=kernel_regularizer)
    self._position_emb = tf.keras.layers.Dense(feed_forward_size, kernel_regularizer=kernel_regularizer)
    self._embedding_dropout = tf.keras.layers.Dropout(embedding_dropout_rate)
    self._output_tokens = tf.keras.layers.Dense(vocab_size, kernel_regularizer=kernel_regularizer)
  ...
  def call(
    ...
    x = self._embedding_dropout(x, training=training)
    scores = []
