...
def EfficientNet(width_coefficient,
                 depth_coefficient,
                 default_size,
                 dropout_rate=dropout_settings.BACKBONE_TOP_DROPOUT,
                 drop_connect_rate=dropout_settings.BACKBONE_DROP_CONNECT,
                 backbone_dropout_rate=dropout_settings.BACKBONE_SPATIAL_DROPOUT,
                 depth_divisor=8,
                 activation='swish',
                 blocks_args='default',
  ...
  Args:
    ...
    backbone_dropout_rate: float, dropout rate after key convolutional blocks.
    ...
    kernel_regularizer: Optional regularizer for the kernel of the layers.
  ...
  for (i, args) in enumerate(blocks_args):
    ...
    for j in range(round_repeats(args.pop('repeats'))):
      ...
      # Add spatial dropout after key blocks (every 2nd block group)
      if backbone_dropout_rate > 0 and i % 2 == 1:
        x = layers.SpatialDropout2D(backbone_dropout_rate, name=f'backbone_dropout_block_{i}_step_{j}')(x)
...
def EfficientNetB3(include_top=True,
  ...
  return maybe_restore_with_film(
      ...
      backbone_dropout_rate=backbone_dropout_rate,
      **kwargs)
