...
from tokenizers import token_learner
...
class RT1ImageTokenizer(tf.keras.layers.Layer):
  ...
  def __init__(self,
    ...
    self._tokenizer = pretrained_efficientnet_encoder.EfficientNetEncoder(
        pooling=False, early_film=True, kernel_regularizer=kernel_regularizer)
    ...
    if self._use_token_learner:
      ...
      self._token_learner = token_learner.TokenLearnerModule(
          ...
          kernel_regularizer=kernel_regularizer)  # dropout_rate uses default from dropout_settings
