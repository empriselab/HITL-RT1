...
class SequencePolicy(actor_policy.ActorPolicy):
  ...
  def get_action_logits(self):
    ...
    return self._actor_network.get_action_logits()
...
class SequenceAgent(tf_agent.TFAgent):
  ...
  def __init__(self,
    ...
    # Extract loss_type from kwargs and remove it to avoid passing to parent class
    ...
    self._actor_network = actor_network(  # pytype: disable=missing-parameter  # dynamic-method-lookup
        ...
        loss_type=loss_type)  # Pass loss_type specifically to the network
    ...
    collect_policy = policy_cls(
        ...
        info_spec=self._info_spec,
        actor_network=self._actor_network,
        training=dropout_settings.ENABLE_INFERENCE_DROPOUT)
    super(SequenceAgent, self).__init__(
        ...
        **kwargs)  # Now kwargs doesn't contain loss_type
  ...
  def _train(self, experience: types.NestedTensor,
    ...
    self._apply_gradients(tape, loss_info.loss)
    return loss_info

  def _apply_gradients(self, tape, loss: types.Tensor):
