...
class TransformerNetwork(network.Network):
  ...
  def __init__(
      ...
      dropout_rate: float = dropout_settings.TRANSFORMER_ATTENTION_DROPOUT,
    ...
    Args:
      ...
      kernel_regularizer: Regularizer for dense kernels.
      ...
      loss_type: Type of loss function ('cross_entropy', 'l1', or 'smooth_l1').
    ...
    self._gaussian_noise_stddev = gaussian_noise_stddev
    ...
    if self._loss_type == 'cross_entropy':
        # Use action tokenizer for discrete tokenization
        ...
        self._action_token_emb = tf.keras.layers.Dense(self._token_embedding_size, kernel_regularizer=kernel_regularizer)
    elif self._loss_type in ('l1', 'smooth_l1'):
        # For L1 loss, create direct regression output layer
        self._action_tokenizer = None
        self._tokens_per_action = 4  # Direct output of 4 coordinates (x1, y1, x2, y2)
        # Create simple regression layer with linear activation to prevent saturation
        self._regression_output = tf.keras.layers.Dense(1, activation=None, name='bbox_regression', kernel_regularizer=kernel_regularizer)
    else:
    ...
    # LOSS_TYPE = 'cross_entropy'  # Options: 'cross_entropy', 'l1'
    # LOSS_TYPE = 'l1'  # Options: 'cross_entropy', 'l1'
    ...
    if self._loss_type == 'cross_entropy':
        ...
        # L1 loss for regression (direct coordinate prediction)
        # For 'smooth_l1' we'll replace with Huber below
        if self._loss_type == 'l1':
            self._loss_object = tf.keras.losses.MeanAbsoluteError(reduction=tf.keras.losses.Reduction.NONE)
        else:
            self._loss_object = tf.keras.losses.Huber(reduction=tf.keras.losses.Reduction.NONE)
    elif self._loss_type == 'smooth_l1':
        # Smooth L1 is Huber loss in TF; default delta=1.0
        self._loss_object = tf.keras.losses.Huber(reduction=tf.keras.losses.Reduction.NONE)
    else:
        raise ValueError(f"Unknown loss type: {self._loss_type}")
    ...
    # Determine action_tokens dtype based on loss type
    action_tokens_dtype = tf.int32 if self._loss_type == 'cross_entropy' else tf.float32
  ...
  def call(self,
    ...
    if outer_rank == 1:  # This is an inference call
      if self._loss_type == 'cross_entropy':
          # Original cross-entropy inference logic
          ...
          # Store action logits for debugging/analysis
          self._action_logits = tf.concat(action_predictions_logits, 1)
          ...
          # L1 loss inference logic - direct regression using a per-token regression head
          ...
          # Gather hidden states for all action tokens: [b, num_action_tokens, 512]
          action_hidden = tf.gather(output_tokens, self._action_tokens_mask - 1, axis=1)
          
          # For inference, we only have 1 timestep, so reshape to [b, 4, 512]
          action_hidden = tf.reshape(action_hidden, [b, self._tokens_per_action, -1])

          # Regress continuous coords: [b, 4, 512] -> [b*4, 512] -> [b*4, 1] -> [b, 4]
          action_hidden_reshaped = tf.reshape(action_hidden, [-1, action_hidden.shape[-1]])  # [b*4, 512]
          continuous_output_reshaped = self._regression_output(action_hidden_reshaped)  # [b*4, 1]
          continuous_output = tf.reshape(continuous_output_reshaped, [b, 4])  # [b, 4]

          self._loss = tf.constant(0.0)

          # Package output
          output_actions = tensorspec_utils.TensorSpecStruct()
          output_actions.bbox = continuous_output
      ...
      if self._loss_type == 'cross_entropy':
          # Original cross-entropy logic with tokenization
          ...
          # Store action logits for debugging/analysis
          self._action_logits = action_logits_for_output
          # predicted_tokens_for_output is [b, self._tokens_per_action]
          predicted_tokens_for_output = tf.argmax(
              action_logits_for_output, axis=-1, output_type=tf.int32)

          num_action_tokens = tf.cast(b * t * self._tokens_per_action, tf.float32)
          action_loss = tf.reduce_sum(self._loss_object(action_tokens, action_logits_for_training)) / num_action_tokens
          ...
          # L1 loss logic with direct regression using the per-token regression head
          # Extract hidden states for all action tokens across timesteps: [b, t*4, 512] gathered then reshaped
          action_hidden = tf.gather(output_tokens, self._action_tokens_mask - 1, axis=1)
          action_hidden = tf.reshape(action_hidden, [b, t, self._tokens_per_action, -1])  # [b, t, 4, 512]

          # Add debug prints for the first few iterations
          if training and tf.random.uniform([]) < 0.01:  # Print ~1% of the time
              tf.print("DEBUG: action_hidden stats - min:", tf.reduce_min(action_hidden), 
                      "max:", tf.reduce_max(action_hidden), 
                      "mean:", tf.reduce_mean(action_hidden),
                      "std:", tf.math.reduce_std(action_hidden))

          # Regress per timestep, per coordinate: [b, t, 4, 512] -> [b*t*4, 512] -> [b*t*4, 1] -> [b, t, 4]
          action_hidden_reshaped = tf.reshape(action_hidden, [-1, action_hidden.shape[-1]])  # [b*t*4, 512]
          continuous_outputs_reshaped = self._regression_output(action_hidden_reshaped)  # [b*t*4, 1]
          continuous_outputs = tf.reshape(continuous_outputs_reshaped, [b, t, 4])  # [b, t, 4]

          # Add debug prints for regression outputs
          if training and tf.random.uniform([]) < 0.01:  # Print ~1% of the time
              tf.print("DEBUG: continuous_outputs stats - min:", tf.reduce_min(continuous_outputs), 
                      "max:", tf.reduce_max(continuous_outputs), 
                      "mean:", tf.reduce_mean(continuous_outputs),
                      "std:", tf.math.reduce_std(continuous_outputs))

          # Use the last timestep for output
          continuous_output = continuous_outputs[:, -1]

          # Compute L1 loss against ground truth coordinates in [0,1]
          # Use ALL timesteps for loss computation (like cross-entropy)
          gt_coordinates_all_timesteps = tf.reshape(action_tokens, [b * t, 4])  # [b*t, 4]
          continuous_outputs_all_timesteps = tf.reshape(continuous_outputs, [b * t, 4])  # [b*t, 4]
          action_loss = tf.reduce_mean(self._loss_object(gt_coordinates_all_timesteps, continuous_outputs_all_timesteps))
          self._loss = action_loss

          # Add debug prints for loss computation
          if training and tf.random.uniform([]) < 0.01:  # Print ~1% of the time
              tf.print("DEBUG: gt_coordinates stats - min:", tf.reduce_min(gt_coordinates_all_timesteps), 
                      "max:", tf.reduce_max(gt_coordinates_all_timesteps), 
                      "mean:", tf.reduce_mean(gt_coordinates_all_timesteps))
              tf.print("DEBUG: action_loss:", action_loss)

          self._aux_info.update({
              'action_predictions': continuous_output,
              'action_labels': tf.reshape(action_tokens[:, -1, :], [b, 4]),  # Use ground truth coordinates from last timestep
              'action_loss': action_loss,
              'actor_loss_mask': tf.ones([b], dtype=tf.float32)
          })
  ...
  def add_summaries(self, observations: dict[str, tf.Tensor],
    ...
    with tf.name_scope('ActionTokens'):
      ...
      # Accuracy across timesteps (only for cross-entropy loss with multiple timesteps)
      if self._loss_type == 'cross_entropy' and len(token_accuracy.shape) == 3:
          for t in range(self._time_sequence_length):
            tf.compat.v2.summary.scalar(
                name='accuracy/time_step/{}'.format(t),
                data=tf.reduce_mean(token_accuracy[:, t, :]),
                step=self._train_step_counter)
      # Handle different loss types for summaries
      if self._loss_type == 'cross_entropy' and self._action_tokenizer is not None:
          # Cross-entropy loss: use action tokenizer for detailed summaries
          token_index = 0
          ...
          # L1 loss: simpler summaries for continuous coordinates
          tf.compat.v2.summary.scalar(
              name='accuracy/bbox_coordinates',
              data=accuracy,
              step=self._train_step_counter)
          # Log histograms for bbox coordinates
          # For L1 loss, action_labels and action_predictions are [batch, 4] not [batch, timesteps, 4]
          for i, coord_name in enumerate(['x1', 'y1', 'x2', 'y2']):
              tf.summary.histogram(
                  'bbox/{}/labels'.format(coord_name),
                  action_labels[:, i],
                  step=self._train_step_counter)
              tf.summary.histogram(
                  'bbox/{}/predictions'.format(coord_name),
  ...
  def _tokenize_images(self, observations, network_state, training):
    ...
    image = preprocessors.convert_dtype_and_crop_images(
        ...
        convert_dtype=True,
        seed=seed,
        add_gaussian_noise=self._add_gaussian_noise,
        gaussian_noise_stddev=self._gaussian_noise_stddev)
    image = tf.reshape(image, [b, input_t, h, w, c])
  ...
  def _tokenize_actions(self, observations, network_state):
    ...
    if outer_rank == 1:  # This is an inference call
      if self._loss_type == 'cross_entropy':
          ...
          # For L1 loss, action_tokens should be continuous coordinates
          action_tokens = tf.squeeze(network_state['action_tokens'], [3, 4])
          seq_idx = tf.reshape(network_state['seq_idx'], [1])[0]
          # Shift actions by 1 to the left for L1 loss as well
          action_tokens = tf.cond(seq_idx == self._time_sequence_length,
                                  lambda: tf.roll(action_tokens, -1, axis=1),
                                  lambda: action_tokens)
      else:
      ...
      if self._actions is None:
        ...
        if self._loss_type == 'cross_entropy':
            ...
            action_tokens = tf.zeros(
                shape=[b, t, self._tokens_per_action], dtype=tf.float32)
        else:
            raise ValueError(f"Unknown loss type: {self._loss_type}")
      else:
        if self._loss_type == 'cross_entropy':
            ...
            # For L1 loss, extract continuous coordinates directly
            # Assuming self._actions contains bbox coordinates
            bbox_coords = self._actions.bbox  # [b, t, 4] or [b, 4]
            if len(bbox_coords.shape) == 2:
                # [b, 4] -> [b, 1, 4]
                bbox_coords = tf.expand_dims(bbox_coords, axis=1)
            action_tokens = bbox_coords  # [b, t, 4]
        else:
            raise ValueError(f"Unknown loss type: {self._loss_type}")
  ...
  def _assemble_input_token_sequence(self, context_image_tokens, action_tokens,
    ...
    if self._loss_type == 'cross_entropy':
        ...
        # For L1 loss, action_tokens are continuous coordinates
        # We need to convert them to embeddings that can be processed by the transformer
        # Create dummy embeddings for action tokens (since we're not using tokenization)
        action_tokens_shape = tf.shape(action_tokens)
        action_tokens = tf.zeros([action_tokens_shape[0], action_tokens_shape[1], 
                                 action_tokens_shape[2], 1, self._token_embedding_size])
    else:
        raise ValueError(f"Unknown loss type: {self._loss_type}")
  ...
  def get_action_logits(self) -> tf.Tensor:
    """Get the action logits for debugging/analysis."""
